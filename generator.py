...
Versão em PT e EN."""
    ...
    def generate_charts(self, efetivo, espera, efetividade):
        ...
        sizes = [max(efetivo, 0.01), max(espera, 0.01)]
        ...
        ax.set_title(f"Efetividade: {efetividade}%", color='white', fontsize=13, fontweight='bold', pad=15)
    ...
    # ─────────────────────────────────────────────
    def create_pdf(self, data, cliente, projeto, atividades, impedimentos,
                   clima_cliente, h_inicio, h_fim, h_espera, h_efetivo,
                   total_h, efetividade, ia_content, chart_path, next_steps_manual=""):
        ...
        pdf_path = f"RDP_{str(data).replace('-', '')}_{cliente.replace(' ', '_')}.pdf"
        pdf.output(pdf_path)
        return pdf_path
